        ...
        <el-table-column prop="updateTime" label="更新时间" show-overflow-tooltip></el-table-column>
				<el-table-column label="操作" width="300">
					<template #default="scope">
						...
						<el-button size="small" text type="primary" @click="handleGenField(scope.row)"><el-icon><ele-EditPen /></el-icon>更新字段</el-button>
...
import {importTable as importTableApi} from "/@/api/system/tools/gen";
...
export default defineComponent({
    ...
    const handleGenField=(row: TableData)=>{
      let msg = '你确定要更新？';
      let tables:string[] = [] ;
      if(row){
        msg = `此操作将更新业务表：“${row.tableName}”相关字段，是否继续?`
        tables = [row.tableName]
      }else{
        msg = `此操作将更新业务表：“${state.tableNames.join(',')}”相关字段，是否继续?`
        tables = state.tableNames
      }
      if(tables.length===0){
        ElMessage.error('请选择要字段的业务表数据。');
        return
      }
      ElMessageBox.confirm(msg, '提示', {
        confirmButtonText: '确认',
