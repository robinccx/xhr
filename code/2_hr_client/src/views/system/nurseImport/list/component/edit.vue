<template>  
  <div class="system-nurseImport-edit">
    <!-- 添加或修改导入对话框 -->
    <el-dialog v-model="isShowDialog" width="769px" :close-on-click-modal="false" :destroy-on-close="true">
      <template #header>
        <div v-drag="['.system-nurseImport-edit .el-dialog', '.system-nurseImport-edit .el-dialog__header']">{{(!formData.nurseId || formData.nurseId==0?'添加':'修改')+'导入'}}</div>
      </template>
...
export default defineComponent({
  name:"apiV1SystemNurseImportEdit",
  components:{    
  },
  props:{    
  },
      ...
      formWrap.validate((valid: boolean) => {
        if (valid) {
          ...
          addNurseImport(state.formData).then(()=>{
            ...
            //修改
          updateNurseImport(state.formData).then(()=>{
