<template>
	<div class="home-container">
		...
		<el-row :gutter="15" class="home-card-three mb15">
			<el-col :xs="24" :sm="14" :md="14" :lg="16" :xl="16">
				<div class="home-card-item">
					<div style="height: 100%" ref="homeLine2Ref"></div>
				</div>
			</el-col>
			<el-col :xs="24" :sm="10" :md="10" :lg="8" :xl="8" class="home-media">
				<div class="home-card-item">
					<div style="height: 100%" ref="homePie2Ref"></div>
				</div>
			</el-col>
		</el-row>
...
export default defineComponent({
	...
	setup() {
		...
		const homeLine2Ref = ref();
		const homePie2Ref = ref();
		...
		const initLineChart = () => {
			...
			let d1 = (state.lineData.data.length>=2) ? state.lineData.data[0] : [];
			let d2 = (state.lineData.data.length>=2) ? state.lineData.data[1] : [];
			let name1= (state.lineData.areaList.length>=2) ? state.lineData.areaList[0] : "新院";
			let name2= (state.lineData.areaList.length>=2) ? state.lineData.areaList[1] : "老院";
			...
			const option = {
				...
				legend: { data: state.lineData.areaList, right: 0 },
				xAxis: {
		...
		const initPieChart = () => {
			...
			// for (var i = 0; i < state.eduData.length; i++) {
			// 	data.push({ name: getname[i], value: getvalue[i] });
			// }
			
			state.eduData.forEach((d: any) => {
				data.push({ name: d.itemName, value: d.itemValue });
		...
		// 折线图
		const initLine2Chart = () => {
			if (!global.dispose.some((b: any) => b === global.homeCharThree)) global.homeCharThree.dispose();
			global.homeCharThree = <any>echarts.init(homeLine2Ref.value, state.charts.theme);
			
			let d1 = (state.lineData.leaveData.length>=2) ? state.lineData.leaveData[0] : [];
			let d2 = (state.lineData.leaveData.length>=2) ? state.lineData.leaveData[1] : [];
			let name1= (state.lineData.areaList.length>=2) ? state.lineData.areaList[0] : "新院";
			let name2= (state.lineData.areaList.length>=2) ? state.lineData.areaList[1] : "老院";
		...
		const initPie2Chart = () => {
			...
			global.homeCharFour = <any>echarts.init(homePie2Ref.value, state.charts.theme);
			var getname = [];
			var data = [];

			state.titleData.forEach((d: any) => {
				data.push({ name: d.itemName, value: d.itemValue });
				getname.push(d.itemName);
			})
		...
		const getHomeData = () => {
			//   loading.value = true
			ListHome().then((res: any) => {
				let list = res.data.total ?? [];
				state.homeOne = list.map((p: any) => ({
					num1: p.itemValue, 
					num2: p.itemValue2, 
					itemName: p.itemName,
					num4: 'fa fa-github-alt',
				...
				state.lineData.areaList = res.data.area.areaList ?? [];				
				state.lineData.data = res.data.area.data ?? [];
				state.lineData.leaveData = res.data.area.leaveData ?? [];
				state.eduData= res.data.edu ?? [];			
				state.titleData= res.data.title ?? [];			
