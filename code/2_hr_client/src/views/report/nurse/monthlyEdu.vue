  ...
  <div class="archive-title-container">
    <el-card shadow="hover">
      ...
      <el-table v-loading="loading" :data="tableData.data">
        ...
        <el-table-column v-for="column in defColumnData" :key="column.columnKey" :prop="column.columnKey"
...
export default defineComponent({
  ...
  setup() {
    ...
    const state = reactive({
      defColumnData: Array<MonthlyTableColumns>,
      tableData: {
        data: [],
        total: 0,
        loading: false,
    ...
    const initTableData = () => {
      listEducation(state.tableData.param).then((res: any) => {
        state.defColumnData = res.data.list.map((item) => {
          const newItem: MonthlyTableColumns = {
            columnKey: "qty_" + item.educationId,
            columnName: item.educationName,
          }
          return newItem
        })
    ...
    const doQuery = () => {
      loading.value = true
      ListMonthlyEdu(state.tableData.param).then((res: any) => {
