<template>  
  <div class="archive-education-edit">
    <!-- 添加或修改学历对话框 -->
    <el-dialog v-model="isShowDialog" width="769px" :close-on-click-modal="false" :destroy-on-close="true">
      <template #header>
        <div v-drag="['.archive-education-edit .el-dialog', '.archive-education-edit .el-dialog__header']">{{(!formData.educationId || formData.educationId==0?'添加':'修改')+'学历'}}</div>
      </template>
      <el-form ref="formRef" :model="formData" :rules="rules" label-width="90px">        
        <el-form-item label="学历" prop="educationName">
          <el-input v-model="formData.educationName" placeholder="请输入学历" />
        </el-form-item>        
...
export default defineComponent({
  name:"apiV1ArchiveEducationEdit",
  components:{    
  },
  props:{    
    statusOptions:{
  ...
  setup(props,{emit}) {    
    ...
    const onSubmit = () => {
      ...
      formWrap.validate((valid: boolean) => {
        if (valid) {
          state.loading = true;
          if(!state.formData.educationId || state.formData.educationId===0){
            //添加
          addEducation(state.formData).then(()=>{
            ...
            //修改
          updateEducation(state.formData).then(()=>{
