<template>  
  <div class="archive-department-edit">
    <!-- 添加或修改科室对话框 -->
    <el-dialog v-model="isShowDialog" width="769px" :close-on-click-modal="false" :destroy-on-close="true">
      <template #header>
        <div v-drag="['.archive-department-edit .el-dialog', '.archive-department-edit .el-dialog__header']">{{(!formData.deptId || formData.deptId==0?'添加':'修改')+'科室'}}</div>
      </template>
  ...
  setup(props,{emit}) {    
    ...
    const onSubmit = () => {
      ...
      formWrap.validate((valid: boolean) => {
        if (valid) {
          state.loading = true;
          if(!state.formData.deptId || state.formData.deptId===0){
            //添加
          addDepartment(state.formData).then(()=>{
            ...
            //修改
          updateDepartment(state.formData).then(()=>{
