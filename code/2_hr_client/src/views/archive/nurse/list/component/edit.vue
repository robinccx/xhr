<template>
  <div class="archive-nurse-edit">
    <!-- 添加或修改护士对话框 -->
    <el-dialog v-model="isShowDialog" width="769px" :close-on-click-modal="false" :destroy-on-close="true">
      <template #header>
        <div v-drag="['.archive-nurse-edit .el-dialog', '.archive-nurse-edit .el-dialog__header']">{{ (!formData.nurseId
          ||
          formData.nurseId == 0 ? '添加' : '修改') + '护士' }}</div>
      </template>
        ...
        <el-row :gutter="15">
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
            <el-form-item label="院区" prop="areaId">
              <el-select v-model="formData.areaId" placeholder="请选择院区" class="w100">
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
            <el-form-item label="科室" prop="deptId">
              <el-select v-model="formData.deptId" placeholder="请选择科室" @click.native="getDepartmentItemsDeptId"
                class="w100">
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
            <el-form-item label="职称" prop="titleId">
              <el-select v-model="formData.titleId" placeholder="请选择职称" @click.native="getTitleItemsTitleId" class="w100">
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
            <el-form-item label="人员类别" prop="staffType">
              <el-select v-model="formData.staffType" placeholder="请选择人员类别" class="w100">
  ...
  setup(props, { emit }) {
    ...
    const openDialog = (row?: NurseInfoData) => {
      ...
      if (row) {
        ...
        getNurse(row.nurseId!).then((res: any) => {
          ...
          data.educationId = '' + data.educationId
    ...
    const onSubmit = () => {
      ...
      formWrap.validate((valid: boolean) => {
        if (valid) {
          state.loading = true;
          if (!state.formData.nurseId || state.formData.nurseId === 0) {
            //添加
            ...
            state.formData.educationId = '' + state.formData.educationId
            state.formData.deptId = '' + state.formData.deptId
            state.formData.titleId = '' + state.formData.titleId
            state.formData.positionId = '' + state.formData.positionId
            //修改
            updateNurse(state.formData).then(() => {
